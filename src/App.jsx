...
const P="#5B3DF5",PL="#EDEBFF",PD="#4527D9",T="#0D9488",TL="#CCFBF1",A="#D97706",AL="#FEF3C7",R="#DC2626",RL="#FEE2E2",B="#2563EB",G="#64748B",DARK="#0F172A";
...
const aiProxy=async(messages,sys)=>{
  try{
    const res=await fetch("/api/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages,system:sys})});
    if(!res.ok)throw new Error();
    const d=await res.json();
    return d.reply||"Keep pushing!";
  }catch{return"Stay consistent!";}
};
const SEED=[
...
const TESTIMONIALS=[
  {name:"Priya Sharma",role:"PM @ Flipkart",text:"Shobhit's AI PM course changed how I think about roadmaps. Got promoted in 3 months.",av:"PS"},
  {name:"Rahul Verma",role:"TPM @ Meesho",text:"Best TPM content in India. The systems thinking module alone is worth 10x the price.",av:"RV"},
  {name:"Anjali Singh",role:"SDE-2 to PM @ Swiggy",text:"Transitioned from engineering to PM in 6 months. Incredible community support.",av:"AS"}
];
const LB=[
...
function getRecs(state){
  ...
  return state.courses.filter(c=>!ids.has(c.id)&&c.status!=="draft").map(c=>({...c,_s:(tags.has(c.tag)?3:0)+c.rating+(c.free?0.5:0)})).sort((a,b)=>b._s-a._s).slice(0,3);
}
...
function AppProvider({children}){
  ...
  useEffect(()=>{
    (async()=>{
      const saved=await db.get("vstig-v4");
      let p=null;
      try{if(saved)p=JSON.parse(saved.value);}catch{}
      dispatch({type:"HYDRATE",p:{courses:p?.courses||SEED,enrollments:p?.enrollments||[],waitlist:p?.waitlist||[],user:p?.user||null}});
    })();
  },[]);
  useEffect(()=>{
    if(!state.hydrated)return;
    db.set("vstig-v4",{courses:state.courses,enrollments:state.enrollments,waitlist:state.waitlist,user:state.user});
...
const reqRole=(u,r)=>u?.role===r;
const bStat=c=>{
  const n=new Date(),l=new Date(c.launch),s=new Date(c.start);
  if(l.toDateString()===n.toDateString()||s.toDateString()===n.toDateString())return "live";
  if(l>n)return "upcoming";
  return "active";
};

function Btn({children,variant,onClick,small,disabled,style}){
  const v=variant||"primary";
  return React.createElement("button",{onClick,disabled,style:{padding:small?"6px 14px":"10px 22px",borderRadius:10,border:"1.5px solid "+(v==="primary"?P:v==="outline"?P:"transparent"),background:v==="primary"?P:"transparent",color:v==="primary"?"#fff":P,fontSize:small?12:14,fontWeight:500,cursor:disabled?"not-allowed":"pointer",opacity:disabled?0.6:1,whiteSpace:"nowrap",...(style||{})}},children);
}
function Tag({c,bg,children,small}){
  return React.createElement("span",{style:{background:bg||PL,color:c||P,fontSize:small?10:11,fontWeight:500,padding:small?"2px 6px":"3px 10px",borderRadius:20}},children);
}
function Card({children,style}){
  return React.createElement("div",{style:{background:"#fff",border:"0.5px solid #e2e8f0",borderRadius:14,padding:"18px 20px",...(style||{})}},children);
}
function Av({initials,size,color,src}){
  const sz=size||36,co=color||P;
  return React.createElement("div",{style:{width:sz,height:sz,borderRadius:"50%",background:co+"22",color:co,fontSize:sz*0.36,fontWeight:500,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,overflow:"hidden"}},
...
function PBar({pct,color}){
  return React.createElement("div",{style:{background:"#f1f5f9",borderRadius:99,height:6,overflow:"hidden"}},
    React.createElement("div",{style:{width:(pct||0)+"%",height:"100%",background:color||P,borderRadius:99,transition:"width 0.6s"}})
  );
}
function Skel({w,h,r}){
  return React.createElement("div",{style:{width:w||"100%",height:h||14,borderRadius:r||6,background:"#f1f5f9",animation:"pulse 1.4s ease-in-out infinite"}});
}
function PDsp({c,large}){
...
function ToastItem({t}){
  ...
  const cols={success:{bg:TL,c:"#065f46"},error:{bg:RL,c:"#7f1d1d"},info:{bg:"#DBEAFE",c:"#1e3a5f"}};
  const col=cols[t.kind]||cols.info;
  return React.createElement("div",{style:{background:col.bg,color:col.c,padding:"10px 16px",borderRadius:10,fontSize:13,fontWeight:500,display:"flex",alignItems:"center",gap:10,minWidth:240,maxWidth:340}},
    React.createElement("span",{style:{flex:1}},t.msg),
    React.createElement("button",{onClick:()=>dispatch({type:"DISMISS_TOAST",id:t.id}),style:{background:"none",border:"none",cursor:"pointer",color:col.c,fontSize:18}},"×")
  );
}
function Countdown({target}){
  ...
  if(!t)return React.createElement("div",{style:{padding:"8px 14px",borderRadius:8,background:RL,color:R,fontSize:12,marginBottom:12}},"Scheduled time has passed");
...
function SpCard({speaker,linkedin,speakerPic,prev}){
  if(!speaker&&!linkedin)return null;
  return React.createElement("div",{style:{padding:"12px 14px",borderRadius:12,background:prev?PL:"#f8fafc",border:"1px solid "+(prev?P+"33":"#e2e8f0"),marginTop:12}},
    prev&&React.createElement("div",{style:{fontSize:10,color:P,fontWeight:500,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}},"Speaker Preview"),
    React.createElement("div",{style:{display:"flex",alignItems:"center",gap:10}},
      React.createElement(Av,{initials:(speaker||"S")[0],size:40,color:P,src:speakerPic||null}),
      ...
      linkedin&&okUrl(linkedin)&&React.createElement("a",{href:linkedin,target:"_blank",rel:"noreferrer",style:{padding:"6px 12px",borderRadius:8,background:"#0A66C2",color:"#fff",fontSize:11,fontWeight:500,textDecoration:"none"}},"LinkedIn")
    )
  );
}
...
function PDPModal(){
  ...
  const max=c.maxSeats!=null?c.maxSeats:null;
  const isFull=max!==null&&seatsUsed>=max;
  const left=max!==null?Math.max(0,max-seatsUsed):null;
  const wl=waitlist.filter(w=>w.courseId===c.id);
  const myWI=waitlist.findIndex(w=>w.userId===user?.id&&w.courseId===c.id);
  const onWL=myWI!==-1;
  const enroll=()=>{
    if(!user){close();return;}
    if(isFull&&!isEnrolled){
      if(onWL){dispatch({type:"TOAST",v:{kind:"info",msg:"You're #"+(myWI+1)+" on the waitlist."}});return;}
      dispatch({type:"ADD_WAITLIST",v:{id:Date.now(),userId:user.id,courseId:c.id,name:user.name,joinedAt:new Date().toISOString()}});
      dispatch({type:"TOAST",v:{kind:"info",msg:"You're #"+(wl.length+1)+" on the waitlist!"}});
      close();return;
    }
    if(c.free||c.price===0){
  ...
  return React.createElement("div",{onClick:e=>e.target===e.currentTarget&&close(),style:{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"60px 16px 40px",background:"rgba(0,0,0,0.5)",overflowY:"auto"}},
    React.createElement("div",{style:{background:"#fff",borderRadius:20,padding:"28px 24px",maxWidth:560,width:"100%",position:"relative"}},
      ...
      React.createElement("div",{style:{display:"flex",gap:14,alignItems:"flex-start",marginBottom:20}},
        ...
        React.createElement("div",{style:{flex:1}},
          React.createElement("div",{style:{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}},
            ...
            React.createElement(SBadge,{status:st})
          ),
          React.createElement("div",{style:{fontSize:18,fontWeight:500,marginBottom:4}},c.title),
          React.createElement("div",{style:{fontSize:12,color:G}},c.weeks+" weeks · "+seatsUsed+" enrolled"+(c.rating?" · ★ "+c.rating:""))
        )
      ),
      st==="upcoming"&&c.start&&React.createElement(Countdown,{target:c.start}),
      ...
      left!==null&&!isEnrolled&&!isFull&&left<=5&&React.createElement("div",{style:{fontSize:11,color:R,marginBottom:8,fontWeight:500}},"Only "+left+" seat"+(left!==1?"s":"")+" left!"),
      isFull&&!isEnrolled&&React.createElement("div",{style:{fontSize:11,color:A,marginBottom:8}},"Batch full · "+wl.length+" on waitlist"),
      React.createElement("div",{style:{display:"flex",gap:12,alignItems:"center",marginBottom:16,flexWrap:"wrap"}},
        React.createElement(PDsp,{c,large:true}),
        ...
        isFull?React.createElement(Btn,{onClick:enroll},onWL?"Waitlisted #"+(myWI+1):"Join Waitlist"):
        React.createElement(Btn,{onClick:enroll},c.free?"Enroll Free":"Buy Now ₹"+(c.offerPrice||c.price||c.mrp).toLocaleString())
      ),
      React.createElement(SpCard,{speaker:c.speaker,linkedin:c.linkedin,speakerPic:c.speakerPic})
    )
  );
}
...
function PayModal(){
  ...
  const pay=async()=>{
    ...
    dispatch({type:"ADD_ENROLLMENT",v:{id:Date.now(),userId:state.user.id,courseId:c.id,enrolledAt:new Date().toISOString(),progress:0,completed:false,paymentRef:"pay_"+Math.random().toString(36).slice(2,9)}});
    dispatch({type:"TOAST",v:{kind:"success",msg:"Enrolled in "+c.title+"!"}});
    setStep("success");setProc(false);
  };
  return React.createElement("div",{onClick:e=>e.target===e.currentTarget&&close(),style:{position:"fixed",inset:0,zIndex:250,display:"flex",alignItems:"center",justifyContent:"center",padding:16,background:"rgba(0,0,0,0.5)"}},
    React.createElement("div",{style:{background:"#fff",borderRadius:20,padding:"28px 24px",maxWidth:420,width:"100%"}},
      step==="checkout"?React.createElement("div",null,
        React.createElement("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:20}},
          React.createElement("div",null,React.createElement("div",{style:{fontSize:16,fontWeight:500,marginBottom:4}},c.title),React.createElement("div",{style:{fontSize:12,color:G}},c.tag)),
          React.createElement("button",{onClick:close,style:{background:"none",border:"none",fontSize:22,cursor:"pointer",color:G}},"×")
        ),
        React.createElement("div",{style:{background:"#f8fafc",borderRadius:12,padding:16,marginBottom:14}},
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:6}},React.createElement("span",null,"MRP"),React.createElement("span",null,"₹"+Number(c.mrp).toLocaleString())),
        ...
        React.createElement("div",{style:{display:"grid",gap:10,marginBottom:14}},
          React.createElement("input",{placeholder:"Card: 4111 1111 1111 1111",style:{padding:"10px 12px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:13}}),
        ...
        React.createElement("button",{onClick:pay,disabled:proc,style:{width:"100%",padding:"13px",borderRadius:12,background:proc?"#9ca3af":DARK,border:"none",color:"#fff",fontSize:14,fontWeight:500,cursor:proc?"not-allowed":"pointer"}},proc?"Processing...":"Pay ₹"+Number(price).toLocaleString())
        ...
        React.createElement("div",{style:{fontSize:13,color:G,marginBottom:20}},"Enrolled in "+c.title),
...
function AuthModal({onClose,onLogin}){
  ...
  const login=role=>{
    const u={id:"u"+Date.now(),name:san(name)||"Learner",avatar,role:role||"student"};
    dispatch({type:"SET_USER",v:u});
    dispatch({type:"TOAST",v:{kind:"success",msg:"Welcome "+u.name+"!"}});
    onClose();
  ...
  return React.createElement("div",{onClick:e=>e.target===e.currentTarget&&onClose(),style:{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:"rgba(15,10,46,0.6)"}},
    React.createElement("div",{style:{width:"100%",maxWidth:420,background:"#fff",borderRadius:24,overflow:"hidden"}},
      React.createElement("div",{style:{background:"linear-gradient(135deg,#1e1048,#3b1fa8)",padding:"28px 28px 24px",position:"relative"}},
        ...
        React.createElement("div",{style:{display:"flex",gap:6}},steps.map((s,i)=>React.createElement("div",{key:s,style:{height:3,borderRadius:99,background:i<=idx?"#a78bfa":"rgba(255,255,255,0.2)",flex:i===idx?2:1}}))),
      ...
      React.createElement("div",{style:{padding:"28px 28px 32px"}},
        step==="login"&&React.createElement("div",null,
          ...
          React.createElement("button",{onClick:()=>login("admin"),style:{width:"100%",padding:"11px",borderRadius:12,border:"1.5px solid #fca5a5",background:"#fff5f5",cursor:"pointer",marginBottom:16,fontSize:13,color:R,fontWeight:500}},"Login as Admin (Demo)"),
          React.createElement("div",{style:{display:"flex",gap:8,marginBottom:12}},
            React.createElement("select",{style:{padding:"11px 10px",borderRadius:10,border:"1.5px solid #e2d9f3",background:"#faf8ff",color:"#1e1048",fontSize:14,width:90,flexShrink:0}},
              React.createElement("option",null,"+91"),React.createElement("option",null,"+1"),React.createElement("option",null,"+44")
            ),
        ...
        step==="otp"&&React.createElement("div",null,
          ...
          React.createElement("div",{style:{display:"flex",gap:8,justifyContent:"center",marginBottom:24}},
            otp.map((v,i)=>React.createElement("input",{key:i,ref:refs[i],maxLength:1,value:v,onChange:e=>hOtp(i,e.target.value),style:{width:50,height:56,textAlign:"center",fontSize:24,fontWeight:600,borderRadius:12,border:"2px solid "+(v?P:"#e2d9f3"),background:v?PL:"#faf8ff",color:"#1e1048",outline:"none"}}))
          ),
          React.createElement("button",{onClick:()=>setStep("profile"),style:{width:"100%",padding:"12px",borderRadius:12,background:P,border:"none",color:"#fff",fontSize:14,cursor:"pointer",marginBottom:12}},"Verify & Continue"),
          React.createElement("div",{style:{textAlign:"center"}},React.createElement("button",{onClick:()=>setStep("login"),style:{background:"none",border:"none",color:P,fontSize:13,cursor:"pointer"}},"Change number"))
        ),
        step==="profile"&&React.createElement("div",null,
          React.createElement("div",{style:{fontSize:20,fontWeight:600,color:"#1e1048",marginBottom:16}},"Complete Profile"),
          React.createElement("div",{style:{background:"#faf8ff",border:"1.5px solid #e2d9f3",borderRadius:14,padding:14,marginBottom:16}},
            React.createElement("div",{style:{fontSize:11,fontWeight:500,color:"#6b6b8a",marginBottom:10,textTransform:"uppercase",letterSpacing:0.5}},"Pick avatar"),
...
function Nav({page,setPage,setShowAuth}){
  ...
  const links=[["courses","Courses"],["about","Founder"]];
  return React.createElement("nav",{style:{position:"sticky",top:0,zIndex:100,background:"rgba(255,255,255,0.97)",backdropFilter:"blur(12px)",borderBottom:"0.5px solid #e2e8f0",padding:"0 24px"}},
    React.createElement("div",{style:{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",gap:8,height:60}},
      ...
      React.createElement("div",{className:"desk-nav",style:{display:"flex",alignItems:"center",gap:4}},
        links.map(([p,l])=>React.createElement("button",{key:p,onClick:()=>setPage(p),style:{padding:"6px 14px",borderRadius:8,border:"none",background:page===p?PL:"transparent",color:page===p?P:G,fontSize:13,cursor:"pointer",fontWeight:page===p?500:400}},l)),
        React.createElement("a",{href:"https://chromewebstore.google.com",target:"_blank",rel:"noreferrer",style:{display:"flex",alignItems:"center",gap:5,padding:"6px 12px",borderRadius:8,border:"1px solid #e2e8f0",background:"#f8fafc",color:"#475569",fontSize:13,fontWeight:500,textDecoration:"none",whiteSpace:"nowrap"}},"AI Tools"),
        user&&React.createElement("button",{onClick:()=>setPage("dashboard"),style:{padding:"6px 14px",borderRadius:8,border:"none",background:page==="dashboard"?PL:"transparent",color:page==="dashboard"?P:G,fontSize:13,cursor:"pointer"}},"Dashboard"),
        reqRole(user,"admin")&&React.createElement("button",{onClick:()=>setPage("admin"),style:{padding:"6px 12px",borderRadius:8,border:"none",background:page==="admin"?RL:"transparent",color:page==="admin"?R:G,fontSize:12,cursor:"pointer"}},"Admin"),
        React.createElement("div",{style:{width:1,height:20,background:"#e2e8f0"}}),
        user?React.createElement("div",{style:{display:"flex",alignItems:"center",gap:8}},
          React.createElement(Av,{initials:user.avatar||user.name?.[0]||"U",size:32,color:P}),
          React.createElement("button",{onClick:()=>{dispatch({type:"LOGOUT"});setPage("home");},style:{fontSize:12,color:G,background:"none",border:"none",cursor:"pointer"}},"Logout")
      ...
      React.createElement("button",{className:"ham",onClick:()=>setMob(o=>!o),style:{display:"none",background:"none",border:"none",cursor:"pointer",padding:4,flexShrink:0}},
    ...
    mob&&React.createElement("div",{style:{padding:"12px 24px 20px",borderTop:"0.5px solid #e2e8f0",display:"flex",flexDirection:"column",gap:4}},
      links.map(([p,l])=>React.createElement("button",{key:p,onClick:()=>{setPage(p);setMob(false);},style:{padding:"10px 14px",borderRadius:8,border:"none",background:"transparent",color:G,fontSize:14,cursor:"pointer",textAlign:"left"}},l)),
      ...
      reqRole(user,"admin")&&React.createElement("button",{onClick:()=>{setPage("admin");setMob(false);},style:{padding:"10px 14px",borderRadius:8,border:"none",background:"transparent",color:R,fontSize:14,cursor:"pointer",textAlign:"left"}},"Admin"),
...
function CCard({c,dashboard}){
  const {state,dispatch}=useApp();
  const {enrollments}=state;
  const st=bStat(c);
  const su=enrollments.filter(e=>e.courseId===c.id).length;
  const max=c.maxSeats!=null?c.maxSeats:null;
  const full=max!==null&&su>=max;
  const left=max!==null?Math.max(0,max-su):null;
  const myE=enrollments.find(e=>e.userId===state.user?.id&&e.courseId===c.id);
  return React.createElement("div",{onClick:()=>dispatch({type:"SET_PDP",v:c}),role:"button",tabIndex:0,onKeyDown:e=>e.key==="Enter"&&dispatch({type:"SET_PDP",v:c}),style:{background:"#fff",border:"0.5px solid #e2e8f0",borderRadius:14,padding:"18px 20px",cursor:"pointer",display:"flex",flexDirection:"column"}},
    React.createElement("div",{style:{height:90,borderRadius:10,background:c.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,marginBottom:14,position:"relative"}},
      c.img,
      st==="live"&&React.createElement("span",{style:{position:"absolute",top:8,right:8,background:R,color:"#fff",fontSize:9,fontWeight:600,padding:"2px 7px",borderRadius:99}},"LIVE"),
      full&&React.createElement("span",{style:{position:"absolute",bottom:8,right:8,background:DARK,color:"#fff",fontSize:9,fontWeight:600,padding:"2px 7px",borderRadius:99}},"Full")
    ),
    ...
    dashboard&&myE&&React.createElement("div",null,
      ...
      React.createElement(PBar,{pct:myE.progress,color:myE.progress>=100?T:c.color}),
      React.createElement("div",{style:{height:8}})
    ),
    React.createElement("div",{style:{flex:1}}),
    React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}},
      React.createElement(PDsp,{c}),
      React.createElement("div",{style:{fontSize:11,color:G}},React.createElement(Star),c.rating,left!==null&&left<=3&&!full&&React.createElement("span",{style:{color:R,marginLeft:4}},"·"+left+" left"))
    )
  );
}
...
function HomePage({setPage,setShowAuth}){
  const {state}=useApp();
  const {courses}=state;
  const feat=courses.filter(c=>c.status!=="draft").slice(0,3);
  const live=courses.filter(c=>bStat(c)==="live");
    ...
    React.createElement("section",{style:{background:DARK,padding:"96px 24px 80px",color:"#fff",textAlign:"center"}},
      React.createElement("div",{style:{maxWidth:700,margin:"0 auto"}},
        live.length>0?
          ...
          React.createElement("div",{style:{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(91,61,245,0.15)",border:"1px solid rgba(139,92,246,0.35)",borderRadius:99,padding:"5px 16px",fontSize:12,color:"#a78bfa",marginBottom:28}}),
        ...
        React.createElement("div",{style:{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap",marginBottom:48}},
          ...
          React.createElement("button",{onClick:()=>setPage("courses"),style:{padding:"13px 28px",borderRadius:12,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",color:"#E2E8F0",fontSize:15,cursor:"pointer"}},"Explore Cohorts")
        ),
        React.createElement("div",{style:{display:"flex",justifyContent:"center",gap:40,flexWrap:"wrap"}},
          [[total.toLocaleString()+"+","learners"],[courses.length+"+","cohorts"],["4.9★","rating"],["₹0","to start"]].map(([v,l])=>
    ...
    React.createElement("div",{style:{maxWidth:1100,margin:"0 auto",padding:"0 24px"}},
      React.createElement("section",{style:{padding:"60px 0 40px"}},
        React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}},
          React.createElement("div",null,
            ...
            React.createElement("div",{style:{fontSize:13,color:G,marginTop:4}},"Live cohorts with Shobhit")
          ),
          React.createElement(Btn,{variant:"outline",small:true,onClick:()=>setPage("courses")},"View all "+courses.length)
        ),
        React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:16}},feat.map(c=>React.createElement(CCard,{key:c.id,c})))
      ),
      React.createElement("section",{style:{background:"linear-gradient(135deg,"+PL+",#f0fdf4)",borderRadius:20,padding:"40px 36px",marginBottom:48,display:"flex",gap:32,alignItems:"center",flexWrap:"wrap"}},
        ...
        React.createElement("div",{style:{flex:1,minWidth:200}},
          ...
          React.createElement("blockquote",{style:{fontSize:18,fontWeight:500,margin:"0 0 8px"}},'"I built Vestigia because I know what it takes to grow from a tier-2 city."'),
          React.createElement("div",{style:{fontSize:13,color:G}},"Shobhit Shubham Saxena · AI, Product & TPM · Founded 2014")
        ),
        ...
        React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14,marginTop:20}},
          TESTIMONIALS.map(t=>React.createElement(Card,{key:t.name},
            React.createElement("div",{style:{display:"flex",gap:10,alignItems:"center",marginBottom:12}},
              React.createElement(Av,{initials:t.av,size:38,color:P}),
              React.createElement("div",null,React.createElement("div",{style:{fontSize:13,fontWeight:500}},t.name),React.createElement("div",{style:{fontSize:11,color:G}},t.role))
            ),
      ...
      React.createElement("section",{style:{paddingBottom:60}},
        ...
        BLOGS.map(b=>React.createElement("div",{key:b.title,style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 0",borderBottom:"0.5px solid #e2e8f0",gap:12,cursor:"pointer"}},
          React.createElement("div",null,
            ...
            React.createElement("div",{style:{display:"flex",gap:10,alignItems:"center"}},React.createElement(Tag,{c:P,bg:PL,small:true},b.tag),React.createElement("span",{style:{fontSize:11,color:G}},b.date+" · "+b.read+" read"))
          ),
      ...
      React.createElement("section",{style:{background:"linear-gradient(135deg,"+P+","+PD+")",borderRadius:20,padding:"48px 36px",textAlign:"center",marginBottom:60,color:"#fff"}},
        ...
        React.createElement("div",{style:{fontSize:14,color:"#c4b5fd",marginBottom:28}},"Join "+total.toLocaleString()+"+ professionals"),
...
function CoursesPage(){
  ...
  const {courses}=state;
  const tags=["All","Free",...[...new Set(courses.map(c=>c.tag))]];
  const shown=courses.filter(c=>{
    const mf=filter==="All"||(filter==="Free"&&c.free)||c.tag===filter;
    const ms=!search||c.title.toLowerCase().includes(search.toLowerCase())||(c.speaker||"").toLowerCase().includes(search.toLowerCase());
  ...
  return React.createElement("div",{style:{maxWidth:1100,margin:"0 auto",padding:"40px 24px"}},
    ...
    React.createElement("div",{style:{fontSize:14,color:G,marginBottom:20}},courses.length+" batches · "+courses.filter(c=>c.free).length+" free"),
    React.createElement("div",{style:{position:"relative",marginBottom:14}},
      React.createElement("input",{value:search,onChange:e=>setSearch(e.target.value),placeholder:"Search courses...",style:{width:"100%",padding:"11px 16px 11px 40px",borderRadius:12,border:"1.5px solid #e2e8f0",fontSize:14,background:"#f8fafc",boxSizing:"border-box"}}),
      React.createElement("span",{style:{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",color:G}},"⌕"),
    ...
    React.createElement("div",{style:{display:"flex",gap:8,marginBottom:24,flexWrap:"wrap"}},
      tags.map(f=>React.createElement("button",{key:f,onClick:()=>setFilter(f),style:{padding:"7px 18px",borderRadius:20,border:"1.5px solid "+(filter===f?P:"#e2e8f0"),background:filter===f?PL:"transparent",color:filter===f?P:G,fontSize:13,cursor:"pointer"}},f))
    ),
    ...
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:16}},shown.map(c=>React.createElement(CCard,{key:c.id,c})))
  );
}

function AboutPage({setPage,setShowAuth}){
  const tl=[{y:"2014",t:"Founded Vestigia",d:"Software for tier-2 cities",c:P},{y:"2018",t:"50+ clients",d:"SMBs, startups, government",c:T},{y:"2021",t:"Pivoted to AI & EdTech",d:"Recognized the talent gap",c:A},{y:"2024",t:"Vestigia Reborn",d:"AI-first LMS platform",c:P},{y:"2025",t:"10,000+ Learners",d:"India's leading EdTech",c:T}];
  return React.createElement("div",{style:{maxWidth:860,margin:"0 auto",padding:"40px 24px"}},
    React.createElement("div",{style:{textAlign:"center",marginBottom:48}},
      ...
      React.createElement("div",{style:{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}},["AI Strategy","Product Management","TPM","EdTech","Systems Thinking"].map(t=>React.createElement(Tag,{key:t,c:P,bg:PL},t)))
    ),
    React.createElement(Card,{style:{marginBottom:28}},
      React.createElement("div",{style:{fontSize:13,fontWeight:500,color:P,textTransform:"uppercase",letterSpacing:1,marginBottom:12}},"My Story"),
      React.createElement("p",{style:{fontSize:14,lineHeight:1.8,color:"#475569",margin:0}},"I grew up in a tier-2 city with big dreams and limited access. The gap wasn't talent — it was opportunity. In 2014 I founded Vestigia to empower people in emerging India."),
      React.createElement("p",{style:{fontSize:14,lineHeight:1.8,color:"#475569",margin:"12px 0 0"}},"After a decade of building software, I launched Vestigia 2.0 — founder-led cohorts with real mentorship and AI-powered learning.")
    ),
    React.createElement("div",{style:{marginBottom:32}},
      React.createElement("h2",{style:{fontSize:18,fontWeight:500,marginBottom:20}},"Journey"),
      tl.map((t,i)=>React.createElement("div",{key:t.y,style:{display:"flex",gap:16,marginBottom:20}},
        React.createElement("div",{style:{display:"flex",flexDirection:"column",alignItems:"center"}},
          React.createElement("div",{style:{width:36,height:36,borderRadius:"50%",background:t.c+"22",color:t.c,fontSize:11,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}},t.y.slice(2)),
        ...
        React.createElement("div",{style:{paddingTop:6,paddingBottom:16}},
          React.createElement("div",{style:{fontSize:14,fontWeight:500}},t.t),
          React.createElement("div",{style:{fontSize:13,color:G}},t.d)
        )
      ))
    ),
    React.createElement("div",{style:{textAlign:"center",padding:"36px",background:PL,borderRadius:16}},
      ...
      React.createElement("div",{style:{fontSize:13,color:G,marginBottom:20}},"Live cohorts, real mentorship"),
...
function Dashboard({setPage}){
  ...
  const [aiChat,setAiChat]=useState([{role:"ai",msg:"Hi! I am your Vestigia AI. Ask about courses or what to study next."}]);
  const [aiLoad,setAiLoad]=useState(false);
  const [aiRem,setAiRem]=useState(AI_DAILY);
  const chatRef=useRef();
  const myE=enrollments.filter(e=>e.userId===user?.id);
  const enrolled=courses.filter(c=>myE.some(e=>e.courseId===c.id));
  const recs=getRecs(state);
  const live=courses.filter(c=>bStat(c)==="live");
  const wl=waitlist.filter(w=>w.userId===user?.id);
  const ip=enrolled.filter(c=>myE.find(e=>e.courseId===c.id)?.progress<100)[0];
  const ipe=ip?myE.find(e=>e.courseId===ip.id):null;
  const sendAI=async()=>{
    if(!aiMsg.trim()||aiLoad)return;
    ...
    setAiLoad(true);
    const sys="You are Vestigia AI. Student: "+user.name+". Enrolled: "+(enrolled.map(c=>c.title).join(", ")||"none")+". Answer in 2-3 sentences.";
    ...
    setAiLoad(false);
    setTimeout(()=>chatRef.current&&chatRef.current.scrollIntoView({behavior:"smooth"}),50);
  };
  const cont=cid=>{
    const e=myE.find(en=>en.courseId===cid);
    if(!e||e.progress>=100)return;
    const np=Math.min(100,e.progress+20);
    dispatch({type:"UPDATE_PROGRESS",userId:user.id,courseId:cid,pct:np});
    dispatch({type:"TOAST",v:{kind:"success",msg:"Progress: "+np+"%!"}});
  };
  return React.createElement("div",{style:{maxWidth:1100,margin:"0 auto",padding:"32px 24px"}},
    (live.length>0||ip)&&React.createElement("div",{style:{background:"linear-gradient(135deg,"+P+"11,"+B+"11)",border:"1.5px solid "+P+"33",borderRadius:14,padding:"16px 20px",marginBottom:24}},
      ...
      live.length>0&&React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:ip?10:0}},
        React.createElement("div",{style:{display:"flex",alignItems:"center",gap:10}},
          React.createElement("span",{style:{width:8,height:8,borderRadius:"50%",background:R,display:"inline-block"}}),
          React.createElement("div",null,React.createElement("div",{style:{fontSize:14,fontWeight:500}},live[0].title),React.createElement("div",{style:{fontSize:12,color:G}},"Live now"))
        ),
        React.createElement(Btn,{small:true,onClick:()=>setPage("live"),style:{background:R,borderColor:R,color:"#fff"}},"Join Live")
      ),
      ip&&React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}},
        React.createElement("div",{style:{flex:1,marginRight:16}},
          React.createElement("div",{style:{fontSize:14,fontWeight:500,marginBottom:6}},"Continue: "+ip.title),
          React.createElement(PBar,{pct:ipe?.progress||0,color:ip.color}),
          React.createElement("div",{style:{fontSize:11,color:G,marginTop:4}},(ipe?.progress||0)+"% complete")
        ),
        React.createElement(Btn,{small:true,onClick:()=>cont(ip.id)},"Continue →")
      )
    ),
    React.createElement("h1",{style:{fontSize:22,fontWeight:500,marginBottom:4}},"Welcome back, "+(user?.name?.split(" ")[0]||"")+" "+(user?.avatar||"")),
    React.createElement("div",{style:{fontSize:13,color:G,marginBottom:24}},"Keep your momentum going!"),
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:24}},
      [["Streak","7 🔥",A],["Study Hours","24h",P],["Enrolled",myE.length,T],["Certs",myE.filter(e=>e.completed).length,B]].map(([l,v,c])=>
        React.createElement("div",{key:l,style:{background:"#f8fafc",borderRadius:12,padding:"14px 16px"}},React.createElement("div",{style:{fontSize:11,color:G,marginBottom:4}},l),React.createElement("div",{style:{fontSize:22,fontWeight:600,color:c}},v))
      )
    ),
    React.createElement(Card,{style:{marginBottom:20}},
      ...
      React.createElement("div",{style:{display:"flex",gap:4,flexWrap:"wrap"}},Array.from({length:28},(_,i)=>React.createElement("div",{key:i,style:{width:28,height:28,borderRadius:6,background:i>=21?A+"cc":"#f1f5f9",border:i===27?"2px solid "+A:"none"}}))),
      React.createElement("div",{style:{fontSize:11,color:G,marginTop:8}},"7-day streak · Longest: 14 days")
    ),
    wl.length>0&&React.createElement(Card,{style:{marginBottom:20,background:AL,border:"1px solid "+A+"33"}},
      ...
      wl.map(w=>{
        ...
        return c?React.createElement("div",{key:w.id,style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"0.5px solid #e2e8f0"}},
          React.createElement("div",{style:{fontSize:13}},c.title),React.createElement(Tag,{c:A,bg:AL},"#"+pos+" in queue")
        ):null;
      })
    ),
    enrolled.length===0&&React.createElement("div",{style:{textAlign:"center",padding:"40px 20px",background:"#f8fafc",borderRadius:14,marginBottom:24}},
      React.createElement("div",{style:{fontSize:32,marginBottom:12}},"📚"),
      React.createElement("div",{style:{fontSize:18,fontWeight:500,marginBottom:8}},"Start your learning journey"),
      React.createElement(Btn,{onClick:()=>setPage("courses"),style:{marginTop:8}},"Browse Courses")
    ),
    recs.length>0&&React.createElement("div",{style:{marginBottom:24}},
      React.createElement("div",{style:{fontSize:16,fontWeight:500,marginBottom:14}},"Recommended for you"),
      React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:12}},recs.map(c=>React.createElement(CCard,{key:c.id,c})))
    ),
    ...
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14,marginBottom:24}},
      enrolled.map(c=>React.createElement(CCard,{key:c.id,c,dashboard:true})),
      enrolled.length>0&&React.createElement("div",{onClick:()=>setPage("courses"),role:"button",tabIndex:0,style:{background:PL+"44",border:"1.5px dashed "+P+"44",borderRadius:14,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:180,cursor:"pointer"}},
        React.createElement("div",{style:{fontSize:28,marginBottom:8}},"+"),React.createElement("div",{style:{fontSize:13,color:P,fontWeight:500}},"Browse more")
      )
    ),
    React.createElement("div",{style:{position:"fixed",bottom:24,right:24,zIndex:50}},
      aiOpen&&React.createElement("div",{style:{position:"absolute",bottom:64,right:0,width:320,background:"#fff",border:"0.5px solid #e2e8f0",borderRadius:16,overflow:"hidden",boxShadow:"0 8px 32px rgba(0,0,0,0.1)"}},
        React.createElement("div",{style:{background:P,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}},
          React.createElement("div",null,React.createElement("div",{style:{color:"#fff",fontSize:14,fontWeight:500}},"Vestigia AI"),React.createElement("div",{style:{color:"#c4b5fd",fontSize:10}},aiRem+" messages left")),
        ...
        React.createElement("div",{style:{height:240,overflowY:"auto",padding:"12px 14px",display:"flex",flexDirection:"column",gap:8}},
          ...
          aiLoad&&React.createElement("div",{style:{display:"flex",justifyContent:"flex-start"}},React.createElement("div",{style:{background:PL,borderRadius:12,padding:"8px 12px",color:P,fontSize:13}},"Thinking...")),
          React.createElement("div",{ref:chatRef})
        ),
        React.createElement("div",{style:{padding:"10px 12px",borderTop:"0.5px solid #e2e8f0",display:"flex",gap:8}},
          ...
          React.createElement("button",{onClick:sendAI,disabled:aiLoad||aiRem<=0,style:{width:34,height:34,borderRadius:8,background:P,border:"none",color:"#fff",cursor:"pointer",fontSize:16,opacity:aiLoad?0.6:1}},"↑")
        )
      ),
...
function LiveClass({setPage}){
  ...
  const opts=[{l:"RICE",v:52},{l:"MoSCoW",v:28},{l:"Kano",v:20}];
  const chatRef=useRef();
  const send=()=>{
    if(!msg.trim())return;
  ...
  if(!started)return React.createElement("div",{style:{maxWidth:800,margin:"60px auto",padding:"40px 24px",textAlign:"center"}},
    React.createElement("div",{style:{fontSize:32,marginBottom:12}},"⏱"),
    React.createElement("div",{style:{fontSize:20,fontWeight:500,marginBottom:8}},"Class starts soon"),
    React.createElement("div",{style:{fontSize:13,color:G,marginBottom:24}},"AI PM Masterclass — Module 4: Metrics & OKRs"),
    React.createElement("div",{style:{display:"flex",gap:10,justifyContent:"center",marginBottom:24}},
      ...
      React.createElement(Btn,{variant:"outline",onClick:()=>setPage("dashboard")},"← Back")
    )
  );
  return React.createElement("div",{style:{maxWidth:1100,margin:"0 auto",padding:"20px 24px"}},
    React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
      React.createElement("div",null,
        ...
        React.createElement("div",{style:{fontSize:12,color:G}},"Shobhit Saxena · 347 watching")
      ),
    ...
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 300px",gap:16}},
      React.createElement("div",null,
        React.createElement("div",{style:{background:"#0f0a2e",borderRadius:14,aspectRatio:"16/9",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",position:"relative",marginBottom:12}},
          React.createElement("div",{style:{textAlign:"center"}},
            React.createElement("div",{style:{fontSize:15,color:"#c4b5fd",marginBottom:8}},"Zoom SDK embedded in production"),
            React.createElement("div",{style:{fontSize:13,color:"#8b7cc8"}},"Module 4")
          ),
        ...
        React.createElement(Card,null,
          ...
          opts.map(o=>React.createElement("div",{key:o.l,onClick:()=>!pollVote&&setPollVote(o.l),style:{marginBottom:8,cursor:pollVote?"default":"pointer"}},
      ...
      React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:10}},
        React.createElement("div",{style:{display:"flex",borderRadius:10,overflow:"hidden",border:"0.5px solid #e2e8f0"}},
          ["chat","board"].map(t=>React.createElement("button",{key:t,onClick:()=>setTab(t),style:{flex:1,padding:"8px",border:"none",background:tab===t?P:"#f8fafc",color:tab===t?"#fff":G,fontSize:12,cursor:"pointer"}},t==="chat"?"Chat":"Board"))
        ),
        tab==="chat"&&React.createElement(Card,{style:{padding:0,overflow:"hidden"}},
          React.createElement("div",{ref:chatRef,style:{height:260,overflowY:"auto",padding:"12px 14px",display:"flex",flexDirection:"column",gap:8}},
            chat.map((m,i)=>React.createElement("div",{key:i,style:{display:"flex",gap:8,alignItems:"flex-start"}},
              React.createElement(Av,{initials:m.av,size:28,color:m.isMe?P:T}),
              React.createElement("div",null,
        ...
        tab==="board"&&React.createElement(Card,null,
        ...
        React.createElement(Card,null,
          ...
          React.createElement("div",{style:{display:"flex",gap:6,flexWrap:"wrap"}},
            ["🔥","👏","💡","❤️","🚀","👍"].map(r=>React.createElement("button",{key:r,onClick:()=>dispatch({type:"TOAST",v:{kind:"info",msg:"Reacted "+r}}),style:{fontSize:20,background:"#f8fafc",border:"0.5px solid #e2e8f0",borderRadius:8,padding:"6px 8px",cursor:"pointer"}},r))
          )
        )
      )
    )
...
const EF={title:"",tag:"",mrp:"",offerPrice:"",launch:"",start:"",speaker:"",linkedin:"",speakerPic:"",free:false};
const IS2={width:"100%",padding:"9px 12px",borderRadius:8,border:"1.5px solid #e2e8f0",fontSize:13,background:"#f8fafc",color:"#1e293b",boxSizing:"border-box"};
function Lbl({t,req,note}){
  return React.createElement("label",{style:{fontSize:11,fontWeight:500,color:G,display:"block",marginBottom:4}},
    t,req&&React.createElement("span",{style:{color:R}}," *"),note&&React.createElement("span",{style:{fontSize:10,color:T,fontWeight:400}}," — "+note)
  );
}

function AdminPanel(){
  const {state,dispatch}=useApp();
  if(!reqRole(state.user,"admin"))return React.createElement("div",{style:{maxWidth:600,margin:"80px auto",textAlign:"center",padding:40}},
  ...
  const rs=k=>({...IS2,border:"1.5px solid "+(errs[k]?R:"#e2e8f0")});
  const validate=()=>{
    const e={};
  ...
  const startEdit=c=>{
    ...
    setEditId(c.id);setErrs({});setConf(null);
  };
  const cancel=()=>{setEditId(null);setForm(EF);setErrs({});};
  const save=()=>{
    if(!validate())return;
    const f={title:san(form.title),tag:san(form.tag),mrp:Number(form.mrp),price:Number(form.offerPrice||form.mrp),offerPrice:form.offerPrice?Number(form.offerPrice):null,free:!!form.free,speaker:san(form.speaker),linkedin:okUrl(form.linkedin)?form.linkedin:"",speakerPic:okUrl(form.speakerPic)?form.speakerPic:"",launch:form.launch,start:form.start};
    if(editId){dispatch({type:"UPDATE_COURSE",id:editId,v:f});dispatch({type:"TOAST",v:{kind:"success",msg:"Updated!"}});setEditId(null);}
    else{const nb={...f,id:Date.now(),enrolled:0,rating:0,img:"📚",color:P,status:"upcoming",weeks:6,modules:[],maxSeats:50};dispatch({type:"ADD_COURSE",v:nb});dispatch({type:"TOAST",v:{kind:"success",msg:"Batch created!"}});setSaved(true);setTimeout(()=>setSaved(false),3000);}
    setForm(EF);setErrs({});
  };
  const remove=c=>{dispatch({type:"REMOVE_COURSE",id:c.id});dispatch({type:"TOAST",v:{kind:"info",msg:"Removed."}});setConf(null);if(editId===c.id)cancel();};
  const total=courses.reduce((s,c)=>s+c.enrolled,0);
  const paidE=enrollments.filter(e=>{const c=courses.find(x=>x.id===e.courseId);return c&&!c.free;});
  const rev=paidE.reduce((s,e)=>{const c=courses.find(x=>x.id===e.courseId);return s+(c?.offerPrice||c?.price||0);},0);
  return React.createElement("div",{style:{maxWidth:1100,margin:"0 auto",padding:"28px 24px"}},
    React.createElement("div",{style:{display:"flex",alignItems:"center",gap:12,marginBottom:24}},
      ...
      React.createElement("div",null,React.createElement("div",{style:{fontSize:18,fontWeight:500}},"Admin Panel"),React.createElement("div",{style:{fontSize:12,color:G}},"Vestigia Technologies")),
    ...
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:24}},
      [["Learners",total.toLocaleString(),P],["Batches",courses.length,T],["Paid Enrollments",paidE.length,A],["Revenue","₹"+rev.toLocaleString(),B]].map(([l,v,c])=>
        React.createElement("div",{key:l,style:{background:"#f8fafc",borderRadius:12,padding:"14px 16px"}},React.createElement("div",{style:{fontSize:11,color:G,marginBottom:4}},l),React.createElement("div",{style:{fontSize:18,fontWeight:600,color:c}},v))
      )
    ),
    React.createElement("div",{style:{display:"flex",borderRadius:10,overflow:"hidden",border:"0.5px solid #e2e8f0",marginBottom:20}},
      [["batches","Batches"],["schedule","Schedule"],["analytics","Analytics"]].map(([k,l])=>React.createElement("button",{key:k,onClick:()=>setAtab(k),style:{flex:1,padding:"10px 8px",border:"none",background:atab===k?P:"#f8fafc",color:atab===k?"#fff":G,fontSize:11,cursor:"pointer",fontWeight:atab===k?500:400}},l))
    ),
    atab==="batches"&&React.createElement("div",null,
      React.createElement(Card,{style:{marginBottom:16}},
        React.createElement("div",{style:{fontSize:14,fontWeight:500,marginBottom:4,display:"flex",justifyContent:"space-between",alignItems:"center"}},
          React.createElement("span",{style:{color:editId?A:"#1e293b"}},editId?"Editing Batch":"Create New Batch"),
          editId&&React.createElement(Btn,{small:true,variant:"outline",onClick:cancel},"Cancel")
        ),
        ...
        Object.keys(errs).length>0&&React.createElement("div",{style:{padding:"10px",borderRadius:8,background:RL,color:R,fontSize:13,marginBottom:12}},"Please fix highlighted fields."),
        saved&&React.createElement("div",{style:{padding:"10px",borderRadius:8,background:TL,color:"#065f46",fontSize:13,marginBottom:12}},"Batch published!"),
        React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}},
          ...
          React.createElement("div",null,React.createElement(Lbl,{t:"Tag",req:true}),React.createElement("input",{value:form.tag,onChange:e=>fi("tag",e.target.value),placeholder:"e.g. AI",style:rs("tag")}))
        ),
        React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}},
          ...
          React.createElement("div",null,React.createElement(Lbl,{t:"Offer Price",note:"optional"}),React.createElement("input",{type:"number",value:form.offerPrice,onChange:e=>fi("offerPrice",e.target.value),placeholder:"4999",style:IS2}))
        ),
        React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}},
          React.createElement("div",null,React.createElement(Lbl,{t:"Launch Date",req:true}),React.createElement("input",{type:"date",value:form.launch,onChange:e=>fi("launch",e.target.value),style:rs("launch")})),
          React.createElement("div",null,React.createElement(Lbl,{t:"Schedule",req:true}),React.createElement("input",{type:"datetime-local",value:form.start,onChange:e=>fi("start",e.target.value),style:rs("start")}))
        ),
        form.start&&React.createElement(Countdown,{target:form.start}),
        React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}},
          React.createElement("div",null,React.createElement(Lbl,{t:"Speaker Name"}),React.createElement("input",{value:form.speaker,onChange:e=>fi("speaker",e.target.value),placeholder:"Shobhit Saxena",style:IS2})),
          React.createElement("div",null,React.createElement(Lbl,{t:"LinkedIn URL"}),React.createElement("input",{value:form.linkedin,onChange:e=>fi("linkedin",e.target.value),placeholder:"https://linkedin.com/in/...",style:rs("linkedin")}))
        ),
        React.createElement("div",{style:{display:"flex",alignItems:"center",gap:12}},
          React.createElement("label",{style:{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}},React.createElement("input",{type:"checkbox",checked:form.free,onChange:e=>fi("free",e.target.checked)})," Free"),
          React.createElement("div",{style:{flex:1}}),
          React.createElement(Btn,{small:true,variant:"outline",onClick:()=>{setForm(EF);setErrs({});}},"Clear"),
          React.createElement(Btn,{small:true,onClick:save},editId?"Update":"+ Create")
        )
      ),
          ...
          return React.createElement("div",{key:c.id},
            React.createElement("div",{style:{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",background:"#f8fafc",borderRadius:10,flexWrap:"wrap"}},
              ...
              React.createElement("div",{style:{flex:1,minWidth:140}},
                ...
                React.createElement("div",{style:{fontSize:11,color:G,display:"flex",gap:8,marginTop:2}},React.createElement(Tag,{c:c.color,bg:c.color+"22",small:true},c.tag),React.createElement("span",null,su+"/"+(c.maxSeats||"∞")))
              ),
              React.createElement(SBadge,{status:bStat(c)}),
              React.createElement("div",{style:{fontSize:13,fontWeight:500}},c.free?React.createElement("span",{style:{color:T}},"Free"):c.offerPrice?React.createElement("span",null,React.createElement("span",{style:{color:T}},"₹"+Number(c.offerPrice).toLocaleString()),React.createElement("span",{style:{fontSize:11,color:G,textDecoration:"line-through",marginLeft:4}},"₹"+Number(c.mrp).toLocaleString())):React.createElement("span",{style:{color:P}},"₹"+Number(c.mrp||0).toLocaleString())),
              React.createElement("div",{style:{display:"flex",gap:6,alignItems:"center"}},
                ...
                conf===c.id?
                  React.createElement("span",{style:{display:"flex",gap:4,alignItems:"center"}},
                    React.createElement("span",{style:{fontSize:11,color:R}},"Sure?"),
                    React.createElement(Btn,{small:true,onClick:()=>remove(c),style:{background:R,borderColor:R,color:"#fff"}},"Yes"),
                    React.createElement(Btn,{small:true,variant:"outline",onClick:()=>setConf(null)},"No")
                  ):
                  React.createElement(Btn,{small:true,onClick:()=>setConf(c.id),style:{background:RL,color:R,border:"none"}},"Remove")
              )
            )
          );
        })
    ...
    atab==="schedule"&&React.createElement(Card,null,
      ...
      React.createElement("div",{style:{display:"grid",gap:12}},
        React.createElement("select",{style:IS2},courses.map(c=>React.createElement("option",{key:c.id},c.title))),
        React.createElement("input",{placeholder:"Session title",style:IS2}),
        React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}},React.createElement("input",{type:"datetime-local",style:IS2}),React.createElement("input",{placeholder:"Zoom ID",style:IS2})),
        React.createElement(Btn,{onClick:()=>dispatch({type:"TOAST",v:{kind:"success",msg:"Session scheduled!"}})},"Schedule & Notify")
      )
    ),
    atab==="analytics"&&React.createElement(Card,null,
      React.createElement("div",{style:{fontSize:14,fontWeight:500,marginBottom:14}},"Enrollments by batch"),
      courses.map(c=>{
        const mx=Math.max(...courses.map(x=>x.enrolled),1);
        return React.createElement("div",{key:c.id,style:{marginBottom:10}},
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:3}},React.createElement("span",{style:{color:"#475569"}},c.title),React.createElement("span",{style:{fontWeight:500,color:c.color}},c.enrolled.toLocaleString())),
          React.createElement("div",{style:{background:"#f1f5f9",borderRadius:99,height:6,overflow:"hidden"}},React.createElement("div",{style:{width:Math.round((c.enrolled/mx)*100)+"%",height:"100%",background:c.color,borderRadius:99}}))
        );
      })
    )
  );
  ...
  return React.createElement("footer",{style:{borderTop:"0.5px solid #e2e8f0",padding:"32px 24px",marginTop:24}},
    React.createElement("div",{style:{maxWidth:1100,margin:"0 auto",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:24}},
      React.createElement("div",null,
        ...
        React.createElement("div",{style:{fontSize:12,color:G,lineHeight:1.7,maxWidth:200}},"Bridging talent and technology.")
      ),
      [["Platform",["Courses","Dashboard"]],["Company",["About","Blog"]],["Legal",["Privacy","Terms"]]].map(([t,links])=>
...
class ErrB extends React.Component{
  ...
  render(){
    if(this.state.err)return React.createElement("div",{style:{padding:"40px 24px",textAlign:"center"}},
      ...
      React.createElement("button",{onClick:()=>this.setState({err:null}),style:{padding:"8px 20px",borderRadius:8,background:P,border:"none",color:"#fff",cursor:"pointer",fontSize:13}},"Try again")
    );
...
function AppContent(){
  ...
  const go=useCallback(p=>{
    if(p==="admin"&&!reqRole(state.user,"admin")){dispatch({type:"TOAST",v:{kind:"error",msg:"Admin access required."}});return;}
    if((p==="dashboard"||p==="live")&&!state.user){setShowAuth(true);return;}
    setPage(p);
    if(window.scrollTo)window.scrollTo(0,0);
  },[state.user]);
  if(!state.hydrated)return React.createElement("div",{style:{fontFamily:"system-ui,sans-serif",minHeight:"100vh",background:"#f8fafc"}},
    ...
    React.createElement("div",{style:{maxWidth:1100,margin:"0 auto",padding:"60px 24px"}},
      React.createElement("div",{style:{marginBottom:20}},React.createElement(Skel,{h:28,w:"40%"})),
      React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:16}},
        [1,2,3].map(i=>React.createElement("div",{key:i,style:{background:"#fff",border:"0.5px solid #e2e8f0",borderRadius:14,padding:"18px 20px"}},React.createElement(Skel,{h:90,r:10}),React.createElement("div",{style:{height:12}}),React.createElement(Skel,{h:14,w:"65%"})))
      )
    )
  );
  return React.createElement("div",{style:{fontFamily:"system-ui,-apple-system,sans-serif",minHeight:"100vh",background:"#fff",color:"#1e293b"}},
    ...
    React.createElement(PayModal,null),
    ...
    React.createElement(ErrB,null,
      ...
      page==="admin"&&reqRole(state.user,"admin")&&React.createElement(AdminPanel,null)
    ),
